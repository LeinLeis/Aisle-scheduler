...
- `estimation_method`: `case_rate` (default) or `fixed_duration` (juice, dairy, frozen)
- `fixed_headcount`: only meaningful for `fixed_duration` zones — `None` means "use the juice default of 2." Dairy and Frozen set this explicitly (3 and 2) since their headcount is a fixed daily staffing decision, not something case volume drives.
...
- `fixed_duration` zones (juice) skip both `total_hours` and the capacity math — `workload_hours` is just the flat editable value regardless of who's assigned or how skewed the case count is. **That starting value (~3 hrs) is a placeholder, not an accurate figure** — because juice's `actual_vs_estimate_ratio` is duration-based (not case-count-based), it's a clean, direct signal for replacing that placeholder with a real measured average once enough nights of Complete-check data accumulate. No case math needed to fix this one — just data over time.
- `water_pallet_cases` (juice zone only): **computed, not guessed** — `Σ (pallet_count_by_type × cases_per_pallet)`, using the `PalletType` reference table below. The only nightly manual input is the **pallet count per type**, which is an exact, countable number (you can literally count pallets on the dock), not an estimate of cases. `adjusted_case_count = case_count − water_pallet_cases`. This doesn't affect juice's own `workload_hours` (which never used case count to begin with), but it keeps the raw case number honest for display, and — critically — keeps water pallets from silently inflating any future *pooled* case-rate calibration that might average case-rate data across zones to refine `case_per_minute_rate`.
...
- `dairy_fixed_duration_hours` = 4 hrs, `frozen_fixed_duration_hours` = 4 hrs (both editable placeholders pending real completion-time data, same spirit as juice's ~3 hr placeholder)
...
7. Run the app for a few weeks to accumulate real `actual_vs_estimate_ratio` data, then replace every placeholder figure with a measured one: `case_per_minute_rate`, `needs_improvement_efficiency_factor`, `new_hire_efficiency_factor`, and especially the juice zone's ~3 hr `fixed_duration` — none of these should stay guesses longer than necessary.
8. Research and populate the `PalletType` reference table (cases-per-pallet for each water pallet type) so `water_pallet_cases` is computed, not estimated.
